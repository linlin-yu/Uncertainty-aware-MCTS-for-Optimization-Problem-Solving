...
def run_single_case_dir_gpt(
    ...
    with open(full_log_path, "w", encoding="utf-8") as log_file:
        ...
        response_generator = DirectGPT(api_key=api_key, model=model)
        # LLM evaluator
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", default="IndustryOR", type=str, help="choose dataset"
    )
    parser.add_argument(
        "--openAI_key",
    ...
    parser.add_argument("--log_name", default="direct-ask", type=str)
    ...
    with jsonlines.open(f"testset/testset_{args.dataset}.jsonl") as reader:
        for item in reader:
            ...
            if args.flag_low <= problem_id <= args.flag_high:
                ...
                is_correct, is_executable, result_note = run_single_case_dir_gpt(
